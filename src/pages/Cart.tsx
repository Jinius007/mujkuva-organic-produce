        ...
        try {
          ...
          if (result.error) {
            ...
            if (result.error.message?.includes('does not exist') || result.error.code === '42P01') {
              console.error('   ⚠️  LIKELY ISSUE: mujkuva_organic_orders table does not exist!');
              console.error('   SOLUTION: Run the SQL script: supabase/create-mujkuva-organic-orders.sql');
            }
          } else {
          ...
          console.error(`❌ Exception while saving RESERVED order for ${item.name}:`, error);
